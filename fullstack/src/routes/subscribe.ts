...
export const subscribeRoute = new Hono<{ Variables: Variables }>();
...
subscribeRoute.post('/', async (c) => {
  try {
    ...
    if (!user || !user.userId) {
      return c.json({ status: 'error', message: '请先登录' } as SubscribeResponse, 401);
    }
    ...
    if (!code) {
      return c.json({ status: 'error', message: '请指定股票代码' } as SubscribeResponse, 400);
    }
    ...
    if (!stock) {
      return c.json({ status: 'error', message: '股票不存在' } as SubscribeResponse, 404);
    }
    ...
    return c.json({ status: 'error', message: '订阅失败，请稍后重试' } as SubscribeResponse, 500);
  }
});
...
subscribeRoute.delete('/', async (c) => {
  try {
    ...
    if (!code) {
      return c.json({ status: 'error', message: '参数不完整' } as SubscribeResponse, 400);
    }
    ...
    return c.json({ status: 'error', message: '取消订阅失败' } as SubscribeResponse, 500);
  }
});
...
subscribeRoute.get('/my', async (c) => {
  try {
    ...
    const list = result.results.map(row => ({
      ...
      name: row.name,
      market: row.market,
      daily: !!(row.frequency_daily as number),
      weekly: !!(row.frequency_weekly as number),
      alert: !!(row.frequency_alert as number),
