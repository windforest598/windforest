...
export interface PMQDResult {
  pmqd_total: number;
  pmqd_P: { score: number; max: number; label: string };
  pmqd_M: { score: number; max: number; label: string };
  pmqd_Q: { score: number; max: number; label: string };
  pmqd_D: { score: number; max: number; label: string };
  stars: string;
  verdict: string;
  ...
  // Safety
  safety: { q1: boolean; q2: boolean; q3: boolean; total: number };
 ...
 */
function computeP(input: PMQDInput): number {
  let score = 0;
  ...
  if (input.div_yield_ttm >= 5) score += 10;
  else if (input.div_yield_ttm >= 4) score += 8;
  else if (input.div_yield_ttm >= 3) score += 6;
  else if (input.div_yield_ttm >= 2) score += 4;
  else if (input.div_yield_ttm >= 1) score += 2;

  // PB discount (max 10)
  if (input.pb <= 0.7) score += 10;
  else if (input.pb <= 1.0) score += 8;
  else if (input.pb <= 1.5) score += 6;
  else if (input.pb <= 2.0) score += 4;
  else if (input.pb <= 3.0) score += 2;
 ...
 */
function computeM(input: PMQDInput): number {
  let score = 0;

  // Industry catalyst (max 9)
  if (input.catalyst_strength >= 80) score += 9;
  else if (input.catalyst_strength >= 60) score += 7;
  else if (input.catalyst_strength >= 40) score += 5;
  else if (input.catalyst_strength >= 20) score += 3;
  else score += 1;

  // Management quality (max 9)
  if (input.management_quality >= 80) score += 9;
  else if (input.management_quality >= 60) score += 7;
  else if (input.management_quality >= 40) score += 5;
  else if (input.management_quality >= 20) score += 3;
  else score += 1;
 ...
 * Q — Quality (0-30)
 */
function computeQ(input: PMQDInput): number {
  let score = 0;

  // ROE (max 15)
  if (input.roe >= 25) score += 15;
  else if (input.roe >= 20) score += 12;
  else if (input.roe >= 15) score += 10;
  else if (input.roe >= 10) score += 7;
  else if (input.roe >= 5) score += 4;
  else score += 0;

  // FCF / Net Profit (max 10)
  const fcfRatio = input.net_profit > 0 ? input.fcf / input.net_profit : 0;
  if (fcfRatio >= 1.0) score += 10;
  else if (fcfRatio >= 0.8) score += 8;
  else if (fcfRatio >= 0.5) score += 5;
  else if (fcfRatio >= 0.3) score += 3;
  else score += 0;

  // Moat depth (max 5)
  if (input.moat_depth >= 80) score += 5;
  else if (input.moat_depth >= 60) score += 4;
  else if (input.moat_depth >= 40) score += 3;
  else if (input.moat_depth >= 20) score += 2;
  else score += 1;
 ...
 */
function computeD(input: PMQDInput): number {
  let score = 0;

  // Market sentiment contrarian indicator (max 6)
  if (input.market_sentiment <= 20) score += 6; // Very bearish = opportunity
  else if (input.market_sentiment <= 40) score += 5;
  else if (input.market_sentiment <= 60) score += 3;
  else if (input.market_sentiment <= 80) score += 1;
  else score += 0; // Euphoric = no discount

  // Net cash discount (max 6)
  const netCashRatio = input.market_cap > 0 ? input.net_cash / input.market_cap : 0;
  if (netCashRatio >= 0.3) score += 6;
  else if (netCashRatio >= 0.2) score += 5;
  else if (netCashRatio >= 0.1) score += 3;
  else if (netCashRatio >= 0.05) score += 2;
  else if (netCashRatio > 0) score += 1;
 ...
 */
function computeKelly(input: PMQDInput, pScore: number, qScore: number): {
  kelly_f: number; kelly_b: number; kelly_p: number; verdict: string;
} {
  ...
  let verdict: string;
  if (kellyF >= 0.3) verdict = '重仓·高置信度';
  else if (kellyF >= 0.15) verdict = '适度仓位·半凯利';
  else if (kellyF >= 0.05) verdict = '小仓位·观察仓';
  else if (kellyF > 0.001) verdict = '微量观察';
  else verdict = '估值偏高·暂不建仓';
 ...
 */
function computeSafety(input: PMQDInput): { q1: boolean; q2: boolean; q3: boolean; total: number } {
 ...
 * Classify strategy
 */
function classifyStrategy(input: PMQDInput): { strategy: string; label: string } {
 ...
 */
export function computePMQD(input: PMQDInput): PMQDResult {
  ...
  else if (total >= 35) stars = '★★☆☆☆';
  else stars = '★☆☆☆☆';

  // Verdict
  let verdict: string;
  if (total >= 80) verdict = '买入级·高置信度';
  else if (total >= 65) verdict = '配置级·重点关注';
  else if (total >= 50) verdict = '观察级·策略二边界';
  else if (total >= 35) verdict = '观望级·条件不足';
  else verdict = '回避级·暂不建议';
