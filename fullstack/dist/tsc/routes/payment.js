...
import { createWechatNativePay, createAlipayPagePay, decryptWechatNotify, verifyAlipayNotify, } from '../services/payment';
...
paymentRoute.post('/create', async (c) => {
    try {
        ...
        const db = getDB(c);
        // 写入支付记录（pending）
        await db.prepare(`INSERT INTO payments (user_id, amount, currency, method, plan, status, out_trade_no)
       VALUES (?, ?, 'CNY', ?, ?, 'pending', ?)`).bind(user.userId, plan.amount / 100, method, plan.name, outTradeNo).run();
        let payUrl;
        let qrCode;
        if (method === 'wechat') {
...
paymentRoute.post('/wechat-notify', async (c) => {
    try {
        ...
        if (trade_state === 'SUCCESS') {
            const db = getDB(c);
            await db.prepare(`UPDATE payments SET status = 'success', paid_at = datetime('now')
         WHERE out_trade_no = ?`).bind(out_trade_no).run();
            // 更新用户订阅计划
            const payRecord = await db.prepare(`SELECT user_id, plan FROM payments WHERE out_trade_no = ? LIMIT 1`).bind(out_trade_no).first();
            if (payRecord) {
                await db.prepare(`UPDATE users SET plan = ?, subscribed_at = datetime('now')
           WHERE id = ?`).bind(payRecord.plan, payRecord.user_id).run();
            }
        }
...
paymentRoute.post('/alipay-notify', async (c) => {
    try {
        ...
        if (tradeStatus === 'TRADE_SUCCESS' || tradeStatus === 'TRADE_FINISHED') {
            const db = getDB(c);
            await db.prepare(`UPDATE payments SET status = 'success', paid_at = datetime('now')
         WHERE out_trade_no = ?`).bind(outTradeNo).run();
            const payRecord = await db.prepare(`SELECT user_id, plan FROM payments WHERE out_trade_no = ? LIMIT 1`).bind(outTradeNo).first();
            if (payRecord) {
                await db.prepare(`UPDATE users SET plan = ?, subscribed_at = datetime('now')
           WHERE id = ?`).bind(payRecord.plan, payRecord.user_id).run();
            }
...
paymentRoute.get('/status', async (c) => {
    try {
        ...
        const db = getDB(c);
        const row = await db.prepare(`SELECT status, plan, paid_at FROM payments WHERE out_trade_no = ? AND user_id = ? LIMIT 1`).bind(outTradeNo, user.userId).first();
        if (!row) {
